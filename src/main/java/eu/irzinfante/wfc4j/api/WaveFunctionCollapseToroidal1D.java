 ...
 * @since	0.1.0
 */
public class WaveFunctionCollapseToroidal1D<T> extends AbstractWaveFunctionCollapse1D<T> {

	/**
	 * Creates a 1-dimensional toroidal grid (last cell touches first cell) on which to apply the WFC algorithm with the specified tilemap
	 *
	 ...
	 * @since	0.1.0
	 */
	public WaveFunctionCollapseToroidal1D(TileMap1D<T> tileMap, int gridSizeX) throws TileException, DimensionException {
	 ...
	 * @since	0.1.0
	 */
	public WaveFunctionCollapseToroidal1D(TileMap1D<T> tileMap, int gridSizeX, long seed) throws TileException, DimensionException {
	...
	protected Cell1D<T> getLeftCell(int x) {
		if(x == 1) {
			return this.getCell(this.grid.length);
		} else {

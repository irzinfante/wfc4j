 ...
 * @since	0.2.0
 */
public class WaveFunctionCollapseEuclidean2D<T> extends AbstractWaveFunctionCollapse2D<T> {
	 ...
	 * @since	0.2.0
	 */
	public WaveFunctionCollapseEuclidean2D(TileMap2D<T> tileMap, int gridSizeX, int gridSizeY) throws TileException, DimensionException {
	 ...
	 * @since	0.2.0
	 */
	public WaveFunctionCollapseEuclidean2D(TileMap2D<T> tileMap, int gridSizeX, int gridSizeY, long seed) throws TileException, DimensionException {
		super(tileMap, gridSizeX, gridSizeY, seed);
	}
	
	@Override
	protected boolean canUpdateLeftCell(int x, int y) {
		return x - 1 >= 1 && this.getLeftCell(x, y).getTile() == null;
	}
	...
	protected boolean canUpdateRightCell(int x, int y) {
		return x + 1 <= this.grid[y - 1].length && this.getRightCell(x, y).getTile() == null;
	}
	...
	protected boolean canUpdateBottomCell(int x, int y) {
		return y + 1 <= this.grid.length && this.getBottomCell(x, y).getTile() == null;
	}
	...
	protected boolean canUpdateTopCell(int x, int y) {
		return y - 1 >= 1 && this.getTopCell(x, y).getTile() == null;
	}
